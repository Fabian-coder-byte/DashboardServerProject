...
function readHostHostname() {
  try {
    return fs.readFileSync("/host/hostname", "utf8").trim();
  } catch (_) {
    return null;
  }
}
...
async function collectSample() {
  try {
    ...
    metricsHistory.push({
      ...
      temp:
        temp.main !== null && temp.main !== undefined
...
router.get("/overview", async (req, res) => {
  try {
    ...
    const ramUsedReal = mem.total - mem.available;
    ...
    res.json({
      ...
      ram: {
        ...
        usedPercent: Math.round((ramUsedReal / mem.total) * 100),
      },
      swap: {
        total: mem.swaptotal,
        used: mem.swapused,
...
router.get("/specs", async (req, res) => {
  try {
    ...
    const manufacturer =
      sys.manufacturer ||
      (hostModel.toLowerCase().includes("raspberry")
...
router.post("/control", async (req, res) => {
  ...
  if (action === "check-updates") {
    try {
      ...
      await execAsync("nsenter -t 1 -m -- apt-get update -qq 2>&1", {
        timeout: 90000,
      }).catch(() => {});
      const { stdout } = await execAsync(
        "nsenter -t 1 -m -- apt list --upgradable 2>/dev/null",
        { timeout: 30000 },
      );
      ...
      return res
        .status(500)
        .json({ error: `Errore nel controllo aggiornamenti: ${err.message}` });
    }
  }
  ...
  if (action === "reboot" || action === "poweroff") {
    ...
    setTimeout(() => {
      ...
      execAsync(cmd).catch((err) =>
        log.error("system", `${action} fallito`, err),
      );
    }, 800);
    return;
  }
  ...
  res
    .status(400)
    .json({ error: "Azione non valida. Usa: reboot, poweroff, check-updates" });
});
